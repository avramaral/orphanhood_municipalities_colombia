...
if (!file.exists(multiplier_file)) {
  ...
  for (l in 1:L) {
    for (y in 1:Y) {
      ...
      # OLD approach: denominator is the standardized death rate at the national level
      # tmp_log_std_nat <- log(c(draws[, paste("std_death_rate_nat[", y, "]", sep = "")]))
      # tmp_mul <- exp(tmp_log_std_mun - tmp_log_std_nat)
      
      # NEW approach: denominator is the standardized death rate at each location, based on the raw data
      tmp_std_death_r <- std_death_rate_mat[l, y]
      # tmp_log_std_loc <- ifelse(tmp_std_death_r == 0, 0, log(tmp_std_death_r)) # CHECK THIS
...
for (l in 1:L) {
  ...
  for (y in 1:Y) {
    for (a in 1:A) {
      ...
      tmp_deaths_fem <- pmax(dth_fem[l, y, a], rbinom(n = sample_size, size = pop_fem[l, y, a], prob = pmin(exp(log_death_rate_fem[y, a, ]), 1)))
      tmp_deaths_mal <- pmax(dth_mal[l, y, a], rbinom(n = sample_size, size = pop_mal[l, y, a], prob = pmin(exp(log_death_rate_mal[y, a, ]), 1)))
