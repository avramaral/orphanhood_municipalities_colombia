...
mort      <- data$mort
mort      <- mort %>% filter(!(gender == "Female" & age %in% c("60-64", "65-69", "70-74", "75-79", "80+")))
mort      <- mort %>% filter(!(gender ==   "Male" & age %in% c(                  "70-74", "75-79", "80+")))
...
fitted_lm <- lm(c(std_death_rate_capital_mat[, 21]) ~ 1 + mpi_capital) # year = 2018
summary(fitted_lm)
