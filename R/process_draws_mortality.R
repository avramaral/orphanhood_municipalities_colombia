...
for (l in 1:L) {
  ...
  for (y in 1:Y) {
    for (a in 1:A) {
      ...
      deaths_fem[y, a, ] <- pmax(dth_fem[l, y, a], rbinom(n = sample_size, size = pop_fem[l, y, a], prob = exp(log_death_rate_fem[y, a, ])))
      deaths_mal[y, a, ] <- pmax(dth_mal[l, y, a], rbinom(n = sample_size, size = pop_mal[l, y, a], prob = exp(log_death_rate_mal[y, a, ])))
    }
  }
