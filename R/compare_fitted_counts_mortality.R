...
fit_deaths <- readRDS(paste("FITTED/DATA/new_new_new_count_mortality_v1_1_empirical_", as.character(use_raw_data), ".RDS", sep = ""))
idx_male   <- which(fit_deaths$Gender == "Male")
...
# fit_deaths <- fit_deaths %>% dplyr::select(year, mun, gender, age, fit_deaths) %>% arrange(year, mun, gender, age)

fit_dth_rt <- readRDS(paste("FITTED/DATA/new_new_new_rates_mortality_v1_1_empirical_", as.character(use_raw_data), ".RDS", sep = ""))
fit_dth_rt[idx_male, "Gender"] <- "Male"
...
fit_dth_rt <- fit_dth_rt %>% left_join(y = mpi_info, by = "mun")
...
p_raw_pts_fem <- plot_std_rate(data = std_raw_fem, tt = "Raw data (mortality) Female", y_lim = c(0, 0.0025))
p_fit_pts_fem <- plot_std_rate(data = std_fit_fem, tt = "Fit data (mortality) Female", y_lim = c(0, 0.0025))
p_tot_pts_fem <- p_raw_pts_fem + p_fit_pts_fem

y_lim <- range(rbind(std_raw_mal, std_fit_mal)[, "std_rate"])
p_raw_pts_mal <- plot_std_rate(data = std_raw_mal, tt = "Raw data (mortality) Male", y_lim = c(0, 0.0025))
p_fit_pts_mal <- plot_std_rate(data = std_fit_mal, tt = "Fit data (mortality) Male", y_lim = c(0, 0.0025))
p_tot_pts_mal <- p_raw_pts_mal + p_fit_pts_mal

# ggsave(filename = paste("IMAGES/std_mortality_points_comparison_fem.jpeg" , sep = ""), plot = p_tot_pts_fem , width = 3000, height = 1500, units = c("px"), dpi = 300, bg = "white")
# ggsave(filename = paste("IMAGES/std_mortality_points_comparison_mal.jpeg" , sep = ""), plot = p_tot_pts_mal , width = 3000, height = 1500, units = c("px"), dpi = 300, bg = "white")
...
# IGNORE THIS ####
std_dth_rate_fit <- compute_std_rate(rename(fit_dth_rt, mortality_rate = death_rate)); std_dth_rate_fit <- std_dth_rate_fit %>% filter(year == y) %>% dplyr::select(-year)
std_dth_rate_fit <- std_dth_rate_fit %>% left_join(y = geo_info[, c("mun", "capital")], by = "mun") %>% left_join(y = mpi_info, by = "mun") %>% mutate(capital = factor(capital), mpi = mpi / 100)
##################

p_fit_pts <- plot_fit_std_rate(data_lin = quantiles_lin, data_fit = std_dth_rate_fit, tt = "Fitted data (mortality)", y_lim = y_lim)

(p_tot_pts <- p_raw_pts + p_fit_pts)
ggsave(filename = paste("IMAGES/std_mortality_points_empirical_", as.character(use_raw_data), "_multiplier_1.jpeg" , sep = ""), plot = p_tot_pts , width = 3000, height = 1500, units = c("px"), dpi = 300, bg = "white")
