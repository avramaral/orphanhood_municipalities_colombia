...
for (l in 1:L) {
  ...
  for (y in 1:Y) {
    for (a in 1:A) {
      ...
      births_fem[y, a, ] <- pmax(brt_fem[l, y, a], rnbinom(n = sample_size, size = tmp_phi_dispe, mu = exp(log_fertility_rate_fem[y, a, ] + log(pop_fem[l, y, a]))))
      births_mal[y, a, ] <- pmax(brt_mal[l, y, a], rnbinom(n = sample_size, size = tmp_phi_dispe, mu = exp(log_fertility_rate_mal[y, a, ] + log(pop_mal[l, y, a]))))
