...
parameters {
  ...
  // Inverse dispersion parameter for Negative Binomial
  vector<lower=0>[G] inv_phi_dispe; 
...
model {
  ...
  // First item collapse in row-major order and the second item collaps in column-major order
  target += neg_binomial_2_log_lpmf(to_array_1d(births_nat_fem) | to_vector(inv_log_fertility_rate_nat_fem[,]'), phi_dispe[1]);
  target += neg_binomial_2_log_lpmf(to_array_1d(births_nat_mal) | to_vector(inv_log_fertility_rate_nat_mal[,]'), phi_dispe[2]);
  ...
  // Hyperparameters
  target += normal_lpdf(inv_phi_dispe | 0, 0.0025); 
...
generated quantities {
  ...
  matrix[Y, A_fem] fertility_rate_nat_fem;
  matrix[Y, A_mal] fertility_rate_nat_mal;

  // Initialise paramters
  std_fertility_rate_nat = rep_vector(0, Y);
  
  for (y in 1:Y) {
    fertility_rate_nat_fem[y, ] = exp(inv_log_fertility_rate_nat_fem[y, ]) ./ to_vector(population_nat_fem[y, ])';
    fertility_rate_nat_mal[y, ] = exp(inv_log_fertility_rate_nat_mal[y, ]) ./ to_vector(population_nat_mal[y, ])';
    // Vectorized sum over age groups
    std_fertility_rate_nat[y] = std_fertility_rate_nat[y] + dot_product(proportion_pop_nat_fem, fertility_rate_nat_fem[y, ]);
    std_fertility_rate_nat[y] = std_fertility_rate_nat[y] + dot_product(proportion_pop_nat_mal, fertility_rate_nat_mal[y, ]);
  }
}
