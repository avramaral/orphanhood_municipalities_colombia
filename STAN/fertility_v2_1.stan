// NATIONAL MODEL (ONLY) WITH POISSON FOR THE NATIONAL COUNT
...
model {
  ...
  target += poisson_log_lpmf(to_array_1d(births_nat_fem[, ]) | log(to_vector(to_array_1d(population_nat_fem[, ]))) + to_vector(inv_log_fertility_rate_nat_fem'));
  target += poisson_log_lpmf(to_array_1d(births_nat_mal[, ]) | log(to_vector(to_array_1d(population_nat_mal[, ]))) + to_vector(inv_log_fertility_rate_nat_mal'));
