...
transformed parameters {
  ...
  // Linear predictor (National level)
  {
    ...
    for (y in 1:Y) {
      inv_log_fertility_rate_nat[1][y, ] = (fertility_rate_baseline[y] + L_fertility_rate_nat_fem * z_fem[y, ]')';                                 // Female
      inv_log_fertility_rate_nat[2][y, ] = (fertility_rate_baseline[y] + fertility_rate_male_effect[y] + L_fertility_rate_nat_mal * z_mal[y, ]')'; // Male
    }
  }
...
model {
  ...
  for (g in 1:G) {
    ...
    target += neg_binomial_2_log_lpmf(to_array_1d(births_nat[, , g]) | log(to_vector(to_array_1d(population_nat[, , g]))) + to_vector(inv_log_fertility_rate_nat[g]'), phi_dispe[g]);
  }
