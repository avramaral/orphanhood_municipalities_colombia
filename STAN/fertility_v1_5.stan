...
parameters {
  ...
  vector<lower=0>[Y] gp_sigma_fem;
  vector<lower=0>[Y] gp_sigma_mal;        
  vector<lower=0>[Y] gp_length_scale_fem;
  vector<lower=0>[Y] gp_length_scale_mal;
  
  // Dispersion parameter for Negative Binomial
  //real<lower=0> phi_dispe; // Dispersion for NB
  matrix<lower=0>[Y, A] phi_dispe_fem; // Dispersion for NB (female)
  matrix<lower=0>[Y, A] phi_dispe_mal; // Dispersion for NB (  male)
...
transformed parameters {
  ...
  {
    array[Y] matrix[A, A] L_fertility_rate_nat_fem;
    array[Y] matrix[A, A] L_fertility_rate_nat_mal;
    
    for (y in 1:Y) {
      L_fertility_rate_nat_fem[y] = cholesky_decompose(gp_exp_quad_cov(age_value, gp_sigma_fem[Y], gp_length_scale_fem[Y]) + diag_matrix(rep_vector(1e-10, A)));
      L_fertility_rate_nat_mal[y] = cholesky_decompose(gp_exp_quad_cov(age_value, gp_sigma_mal[Y], gp_length_scale_mal[Y]) + diag_matrix(rep_vector(1e-10, A)));
      
      inv_log_fertility_rate_nat[1][y, ] = (fertility_rate_baseline[y] + L_fertility_rate_nat_fem[y] * z_fem[y, ]')';                                 // Female
      inv_log_fertility_rate_nat[2][y, ] = (fertility_rate_baseline[y] + fertility_rate_male_effect[y] + L_fertility_rate_nat_mal[y] * z_mal[y, ]')'; // Male
    }
  }
...
model {
  ...
  for (g in 1:G) {
    ...
    if (g == 1) {
      target += neg_binomial_2_log_lpmf(to_array_1d(births_nat[, , g]) | log(to_vector(to_array_1d(population_nat[, , g]))) + to_vector(inv_log_fertility_rate_nat[g]'), to_array_1d(phi_dispe_fem));
    } else {
      target += neg_binomial_2_log_lpmf(to_array_1d(births_nat[, , g]) | log(to_vector(to_array_1d(population_nat[, , g]))) + to_vector(inv_log_fertility_rate_nat[g]'), to_array_1d(phi_dispe_mal));
    }
  }
  ...
  // Hyperparameters
  target += normal_lpdf(to_array_1d(phi_dispe_fem) | 0, 1);
  target += normal_lpdf(to_array_1d(phi_dispe_mal) | 0, 1);
