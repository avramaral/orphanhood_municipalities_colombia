data {
  ...
  // OTHERS
  array[A] int age_value; // Age group values (for GP), between 0 and 1
...
transformed parameters {
  ...
  {
    matrix[A, A] L_death_rate_nat = cholesky_decompose(gp_exp_quad_cov(age_value, gp_sigma, gp_length_scale) + diag_matrix(rep_vector(1e-10, A)));
    
    for (y in 1:Y) {
      inv_logit_death_rate_nat[1][y, ] = (death_rate_baseline[y] + L_death_rate_nat * z_fem[y, ]')';                             // Female
      inv_logit_death_rate_nat[2][y, ] = (death_rate_baseline[y] + death_rate_male_effect[y] + L_death_rate_nat * z_mal[y, ]')'; // Male
    }
  }
...
model {
  ...
  target += inv_gamma_lpdf(gp_length_scale | 5, 1);
